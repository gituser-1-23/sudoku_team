...
extern int selectedRow, selectedCol; // 当前选中的单元格位置
...
// 移除指定数量的数字，生成不同难度的谜题并记录初始状态
void removeNumbers(vector<vector<int>>& board, int count) {
    int removed = 0;
    isInitial.assign(N, vector<bool>(N, true)); // 初始全为非空格
    while (removed < count) {
        int row = rand() % N;
        int col = rand() % N;
        if (board[row][col] != 0) {
            board[row][col] = 0;
            isInitial[row][col] = false; // 标记为空格单元格
            removed++;
        }
    }
}
...
vector<vector<int>> readSudokuFromFile(const string& filename) {
    ...
    isInitial.assign(N, vector<bool>(N, false)); // 默认全为空格单元格
