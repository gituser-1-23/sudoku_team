...
public:
    ...
    static vector<PlayerInfo> loadAllPlayerInfos(const string& folderPath = "./players/") {
        ...
        auto playerFiles = getPlayerNames(folderPath); // 使用之前实现的获取player*文件列表的函数
    ...
    // 通关次数增加方法
    void incrementLevelCount(const string& level) 
    {
        if (level == "simple") simple_mode++;
        else if (level == "mid") mid_mode++;
        else if (level == "hard") hard_mode++;
        else throw invalid_argument("无效的难度等级");
    }
